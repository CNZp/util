/*****************************************************************************
 * ZP前端配置
 * 项目路径在ZP.config.ctxPath上设置，可由ZP.ctxPath /ZP.config.ctxPath 获取
 * 所属项目：plat
 * 最后更新时间：2017-10-12
 * 最后更新人：Zp
 *****************************************************************************/
...
ZP.config = {
    ...
    alias: {
        ...
        'datagrid': '/medias/plugin/zp-grid/datagrid.ZP.1.1.js',
        ...
        "workflow": "/medias/src/plat/code/workflow/instance/workflow.js",
        ...
        "tpl": "/medias/lib/zp/ZP.tpl-0.2.js",
        "perfectLoad": "/medias/plugin/perfectLoad/PerfectLoad.js",
        "button": "/medias/lib/zp/ZP.button-0.1.js"
    },
    ...
    cssAlias: {
        ...
        'datagrid': '/medias/plugin/zp-grid/datagrid.ZP.1.1.css',
        'uploadify': '/medias/plugin/uploadify/uploadify.css',
        'tabPanel': '/medias/plugin/tabPanel/css/TabPanel.css',
        ...
        'tabPanelJb': '/medias/plugin/tabPanel/css/TabPanelJb.css',
        'flow': '/medias/style/plat/workflow/flow.css',
        'myStyle': '/medias/style/plat/workflow/myStyle.css',
        'flowPropertyEdit': '/medias/style/plat/workflow/flowPropertyEdit.css'
    },
    ...
    cssTemplate: {
        ...
        // 'form': ['form'],
        'platMain': ['global', 'platMain', 'iconfont', 'validTip', "model"],
        'platFlow': ['global', 'platFlow', 'iconfont', 'validTip', "model"]
    }
};

//项目路径，ZP命名调整
ZP.ctxPath = ZP.config.ctxPath || "";
...
(function (global) {
    ...
    function _handletemplate(queue, options) {
        var templateQueue = ZP.config.template[options.template], length = templateQueue.length;
        if (templateQueue) {
            for (var i = 0; i < length; i++) {
    ...
    function _handleAlias(alias) {
        var tLocation = ZP.config.alias[alias];
        tLocation = tLocation ? tLocation : alias;
        if (!ZP.browser.canCache) {
    ...
    function _handleCssAlias(alias) {
        var tLocation = ZP.config.cssAlias[alias];
        tLocation = tLocation ? tLocation : alias;
        if (!ZP.browser.canCache) {
            if (tLocation.indexOf("?") > -1) {
                tLocation = tLocation + "&r=" + Math.random();
    ...
    function _loadScript(queue, options) {
        ...
        for (; i < l; i++) {
            ...
            for (; si < sl; si++) {
                var turl = ZP.handlePath(_handleAlias(subqueue[si]));
                if (si == sl - 1) {
                    if (i == l - 1 && options.callback) {
                        loadstr.push(".script('" + turl + "').wait(options.callback)");
                        ZP.browser.debug && console.log("ZP.load:(script)" + turl);
                        ZP.browser.debug && console.log("ZP.load:wait and callback");
                    } else {
                        loadstr.push(".script('" + turl + "').wait()");
                        ZP.browser.debug && console.log("ZP.load:(script)" + turl);
                        ZP.browser.debug && console.log("ZP.load:wait");
                    }
                } else {
    ...
    function _loadBlockJS(options) {
        ...
        options.template && _handletemplate(loadQueue, options);
        options.module && (typeof(options.module) == "object" ? _handleQueue(loadQueue, options) : loadQueue.push([options.module]));
        ZP.loadScriptBlocked(loadQueue);
    }

    // 阻塞式加载css脚本文件
    ZP.loadCssBlocked = function (urls, hasHandleTag) {
        if (urls) {
            if (typeof(urls) == "object") {
                for (var i = 0; i < urls.length; i++) {
                    document.write('<link rel="stylesheet" href="' + (hasHandleTag ? urls[i] : ZP.handlePath(_handleCssAlias(urls[i]))) + '"/>');
                }
            } else if (typeof(urls) == "string") {
                document.write('<link rel="stylesheet" href="' + (hasHandleTag ? urls : ZP.handlePath(_handleCssAlias(urls))) + '"/>');
            }
        }
    }
     ...
     * @return {Object} ZP 链式调用
     */
    ZP.loadCSS = function (options) {
        ZP.handleSrc();
        var loadQueue = [];
        options.template && (loadQueue = loadQueue.concat(ZP.config.cssTemplate[options.template]));
        if (options.module) {
        ...
        for (var i = 0; i < length; i++) {
            loadQueue[i] = ZP.handlePath(_handleCssAlias(loadQueue[i]));
        }
        if (options.async) {
            _loadCSS([].concat(loadQueue), options.callback || function () {
                }, options.obj || {});
        } else {
            ZP.loadCssBlocked([].concat(loadQueue), true);
        }
        //debugger;
        //$(function(){
        //    loadingPages()
     ...
     * @return {Object} ZP 链式调用
     */
    ZP.load = function (options) {
        if (!options || typeof(options) != "object") {
            ZP.browser.debug && console.log("ZP.load:参数异常");
            return;
        }
    ...
    //处理路径
    ZP.handlePath = function (url) {
        if (url) {
            var us = url.toString().split("|");
            if (us.length == 3) {
                if (us[1] != "") {
                    var withPath = ZP.config.paths[us[1]];
                    var resultUrl = us[0] || "";
                ...
                var withPath = ZP.config.ctxPath;
    ...
    // 阻塞式加载js脚本文件
    ZP.loadScriptBlocked = function (urls) {
        if (urls) {
            if (typeof(urls) == "object") {
                for (var i = 0; i < urls.length; i++) {
                    ZP.loadScriptBlocked(urls[i]);
                }
            } else if (typeof(urls) == "string") {
                document.write('<script type="text/javascript" src="' + ZP.handlePath(_handleAlias(urls)) + '"></script>');
            }
        }
    }
    ...
    //操作代码
    //加载默认库js
    ZP.loadScriptBlocked(ZP.config.defaultLib);
