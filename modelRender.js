...
MR.prototype = {
    ...
    variableProperty: function (t, noTip, model) {
        ...
        var type = $(t).prop("type");
        //隐藏元素和禁用元素不验证
        if (!$(t).is(":hidden") && ($(t).prop("disabled") || !$(t).hasClass("disabled"))) {
            ...
            if (value2.type == "file") {
                ...
                //表格类型验证
                    ...
                    if ($.inArray("notNull", value2.rules.checkSave) != -1) {
                        if ($(t).val == null || $(t).val() == "") {
                            result = false;
                            $(t).parent().find(".zpimage").attr("src", "/plat/medias/style/images/plat/002.jpg");
                        }
                    }
                }
     ...
     *  方法：验证字段是否变更
     *****************************************************************/
    variableChange: function (t, model, historyObj) {
        var result = false;
    ...
    fillDict: function (obj, key, value2, model, zdx, key2) {
        if (zdx != null) {
            if (value2.dictConfig.dependence) {
                ...
                model.MR.fillDictByZd(key, zdx, key2, obj, value2, model);
            }
        }
    },
    ...
    fillDictByZd: function (key, zdx, key2, obj, value2, model, pValue) {
        ...
        if (pValue || !value2.dictConfig.dependence) {
            //根据字典生成子项
    ...
    renderProperty: function (t, model) {
        ...
        if (value2.maxLength) {
            ...
            jqueryS.attr("maxlength") != undefined && jqueryS.removeAttr("maxlength");
        }
        ...
        if (value2.type == "layer") {
            ...
            if (value2.layerConfig.type == "frame") {
            ...
            } else if (value2.layerConfig.type == "div") {
                if ($("#_treeAidSelect").length == 0) {
                    var _div = document.createElement("div");
                    _div.id = "_treeAidSelect";
                    _div.className = "thinkDiv";
                ...
                jqueryS.bind("click.ModelEvent", function (event) {
                    var urldata = "";
                    if (value2.layerConfig.checkFunc) {
                        var result = eval(value2.layerConfig.checkFunc + "(model)");
                        if (result !== undefined) {
                    ...
                    var $el = $("#_treeAidSelect");
                    var url;
                    if (value2.layerConfig.url.indexOf("?") > -1) {
                        url = value2.layerConfig.url + "&func=" + value2.layerConfig.callbackFunc + "&modelName=" + modelname + urldata;
                    } else {
                        url = value2.layerConfig.url + "?func=" + value2.layerConfig.callbackFunc + "&modelName=" + modelname + urldata;
                    }
                    $el.find("._treeAidFrame").attr("src", ZP.handlePath(url));
                    var left = $target.offset().left;
                    var top = $target.height() + $target.offset().top;
                    var width = value2.layerConfig.divWidth || ($target.outerWidth() + "px");
                    var height = value2.layerConfig.divHeight || "200px";
                    $el.css({
                        left: left + "px",
                        top: top + "px",
                        width: width,
        ...
        if (tagName == "INPUT") {
            ...
            if (value2.type == "dict") {
                ...
                if (value2.dictConfig.reqInterface) {//自定义获取字典项json接口
                    ...
                    //if(jqueryS.hasClass("disabled")){
                    //    zdx = model.MR.getZdDict(value2.dictConfig.dictCode);
                    //}else{
                    //    zdx = model.MR.getZdDict(value2.dictConfig.dictCode, value2.dictConfig.pcode);
                    //}
                    zdx = model.MR.getZdDict(value2.dictConfig.dictCode, value2.dictConfig.pcode);
                    if (zdx != null) {
                        model.MR.fillDict(jqueryS, model.get("ModelName"), value2, model, zdx, pro);
