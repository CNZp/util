...
var DetailModel = Backbone.RelationalModel.extend({
    ...
    MR: MR(),        //模型渲染器组件实例
    historyObj: null,   //记录页面初始的对象数据，用于判断是否改变
    /*****************************************************************
    ...
    initialize: function (data) {
        ...
        this.beforeRender();
        //缓存使用的字典项
        this.initDict(this.initJson);
    },
    /*
     * 初始处理字典数据
     * */
    initDict: function (objJson) {
        handleCacheDict(objJson);
    },
    /*****************************************************************
     *  方法：需实现接口，设置model名称
     *****************************************************************/
     ...
     * 更新页面加载的数据
     * obj:对象数据
     * 使用场景：使用async或者setTimeout等操作，而且更新了页面的数据，改变的数据dom
     * 比如：满足判断条件，添加和数据相关的dom  ，data-model  data-property
     * */
    updateHistoryData: function () {
        var model = this;
        //记录初始值
        var modelJson = model.getAllJson();
        if (modelJson) {
            model.historyObj = JSON.parse(modelJson);
        } else {
            model.historyObj = null;
        }
    },
    /*********************************************************************************************
     *  方法：对外接口，重新渲染。
     ...
     ************************************************************/
    changeValidate: function (variableJson, checkSelfTag, model, historyObj) {
        var result = false;
        //参数初始值处理
        if (typeof(variableJson) == "boolean") {
            checkSelfTag = variableJson;
        ...
        if (variableAllTag) {
            $("*[data-model=" + model.get("ModelName") + "][data-property]").each(function (i, t) {
                var tresult = model.MR.variableChange(t, model, historyObj);
                ...
                for (var n = 0; n < relations.length; n++) {
                    ...
                    if (relations[n].type == Backbone.HasMany) {
                        if (model.get(tempkey).length > 0) {
                            ...
                            for (var i = 0; i < model.get(tempkey).length; i++) {
                                var tresult;
                                var obj = historyObj[tempkey][i];
                                if (obj) {
                                    tresult = model.changeValidate(variableJson, checkSelfTag, model.get(tempkey).at(i), obj);
                                } else {
                                    tresult = true;
                                }
                                result = result || tresult;
                        ...
                        var tresult;
                        if (model.get(tempkey) != null) {
                            var obj = historyObj[tempkey];
                            if (obj) {
                                tresult = model.changeValidate(variableJson, checkSelfTag, model.get(tempkey), obj);
                            }
                            result = result || tresult;
                        }else{
                            tresult = true;
